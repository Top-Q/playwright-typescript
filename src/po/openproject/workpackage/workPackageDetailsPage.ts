 ...
 *
 * **Status is the exception to that pattern**: it renders as the highlighted
 * status button (`op-wp-status-button`), not as an
 * `.inline-edit--container.status`, and its accessible name is the fixed
 * "Edit the status of the work package" rather than the status value — so a
 * locator built from the inline-edit pattern matches nothing for Status. See
 * {@link getStatus}. OpenProject's own `expect_attributes` spec helper
 * special-cases Status for the same reason.
 *
...
export class WorkPackageDetailsPage extends BasePage<WorkPackageDetailsPage> {
  ...
  private readonly priorityDisplayField: Locator;
  ...
  constructor(public readonly page: Page) {
    ...
    // Status is the one attribute that is NOT an inline-edit container in this
    // view: it renders as the highlighted status button
    // (wp-status-button.html), whose accessible name is the fixed aria-label
    // "Edit the status of the work package" (js.label_edit_status) rather than
    // the status itself - the span holding the status name is aria-hidden. The
    // value is therefore read from the button's text, not from its name.
    this.statusButton = this.detailsPanel
      .getByRole('button', { name: 'Edit the status of the work package' })
      .describe('Status button on the work package details page');
    this.priorityField = this.detailsPanel
      .locator('.inline-edit--container.priority')
      .describe('Priority inline edit container');
    // Same display-field pattern as Assignee: a <span role="button"> whose
    // aria-label reads "Priority <value>: Edit".
    this.priorityDisplayField = this.priorityField
      .getByRole('button', { name: /^Priority/ })
      .describe('Priority display field');
  }
  ...
  /**
   * Returns the Status currently shown for the work package, e.g. 'New' on a
   * freshly created one.
   *
   * Status is rendered as the highlighted status button at the top of the
   * details panel rather than as an inline-edit field, so this reads the
   * button's text. The work packages table on the left of the split view
   * renders a Status cell for every row, so the button is scoped to the details
   * panel — reading it unscoped would pick up a row instead.
   *
   * @aliases status, getStatus, workPackageStatus, readStatus, getStatusName, getWorkPackageStatus, currentStatus, statusValue
   * @prerequisites The work package details view is open
   * @observable-state None — read-only query
   * @returns The status name as displayed, e.g. 'New'.
   */
  async getStatus(): Promise<string> {
    await this.statusButton.waitFor();
    return (await this.statusButton.innerText()).trim();
  }

  /**
   * Returns the Priority currently shown for the work package, e.g. 'Normal'
   * on a freshly created one.
   *
   * Priority is an inline-edit display field in the "Details" attribute group;
   * as with Status, the table on the left renders its own Priority cells, so
   * the field is scoped to the details panel.
   *
   * @aliases priority, getPriority, workPackagePriority, readPriority, getPriorityName, currentPriority, priorityValue
   * @prerequisites The work package details view is open
   * @observable-state None — read-only query
   * @returns The priority name as displayed, e.g. 'Normal'.
   */
  async getPriority(): Promise<string> {
    await this.priorityDisplayField.waitFor();
    return (await this.priorityDisplayField.innerText()).trim();
  }
