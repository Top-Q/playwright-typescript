...
const DELAY_SECS = parseInt(values.delay, 10);
...
const CAPTURE_DOM  = !values.aria || values.dom;
const CAPTURE_ARIA = !values.dom  || values.aria;
...
function cdpSend<T>(wsUrl: string, method: string, params: Record<string, unknown> = {}): Promise<T> {
  return new Promise((resolve, reject) => {
    ...
    ws.onerror = (err) => reject(new Error(`WebSocket error: ${err instanceof Event ? err.type : String(err)}`));
    ws.onclose = () => {};
  });
}
...
function buildAriaYaml(nodes: AXNode[]): string {
  ...
  function formatNode(node: AXNode, indent: number): string {
    ...
    node.properties?.forEach((p) => {
      ...
      props.push(val === true ? p.name : `${p.name}=${JSON.stringify(val)}`);
    });
