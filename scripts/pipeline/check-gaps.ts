...
Exit codes:
  ...
  1  the count did not match, or the scan could not run
...
const ratioMax = values['ratio-max'] === undefined ? undefined : Number(values['ratio-max']);
if (ratioMax !== undefined && !(ratioMax >= 0)) {
  fail(`--ratio-max must be a non-negative number, got "${values['ratio-max']}"`);
}
if (ratioMax !== undefined && values.file === undefined) {
...
function scanTargets(): string[] {
  if (values.file !== undefined) {
    const absolute = path.resolve(repoRoot, values.file);
    if (!fs.existsSync(absolute)) fail(`file not found: ${values.file}`);
    return [absolute];
  }
...
if (values.json) {
  console.log(JSON.stringify({ expected, count, steps, ratio, ratioMax, gaps: hits }, null, 2));
} else if (count > 0) {
...
if (!values.quiet && !values.json) {
  const scope = values.file ?? values.root;
  if (expected === 0) {
