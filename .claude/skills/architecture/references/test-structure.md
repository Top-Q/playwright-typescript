...
- UI tests live in `tests/ui/` organized by feature subdirectories
- API tests live in `tests/api/`
- Test files use `.spec.ts` extension with kebab-case names
- Import `test` from `./fixtures` (not from `@playwright/test`)
- Import `expect` from `@playwright/test`
- Import page objects from `../../internals` (or appropriate relative path)
