...
**Default fixture: `TC-WP-009-01`** — a field change is recorded in the Activity tab.

`workpackage` has 5 classes and (as of 2026-08-13) 23 methods covering create, text-filter,
open, delete, the assignee vocabulary and now `getStatus` / `getPriority` on the details
page. The Activity tab is the nearest surface nothing touches: reaching the work package
resolves entirely from the catalog, and reading the activity entries is a genuine gap.
Sorting (`FR-WP-011`), bulk edit (`FR-WP-012`), attachments (`FR-WP-007`), dates
(`FR-WP-005`) and relations (`FR-WP-008`) are equally uncovered and equally usable.

**Spent fixtures — do not reuse any of these.** Each has a committed test, which fails the
rule at the top of this file:

| Fixture | Consumed by | Test |
|---|---|---|
| `TC-MEM-009-01` | the original members default | `tests/ui/members/filter-members-list-by-role.spec.ts` |
| `TC-WP-006-01` | the first `covered` WP run | `tests/ui/workpackage/assign-work-package-to-a-current-project-member.spec.ts` |
| `TC-WP-003-01` | the `covered` run of 2026-08-12 | `tests/ui/workpackage/new-work-package-defaults-to-status-new-and-priority-normal.spec.ts` |

`TC-MEM-004-01` (the original members fallback) fails the rule from the other direction:
it is now *fully* covered — `MemberTableRowComp` gained `clickManageRoles`, `toggleRole`,
`isRoleChecked` and `clickChangeButton` — so it yields zero gaps and never exercises
po-builder.

Two consequences of the 2026-08-12 run worth knowing before you pick: `FR-WP-003` and
`FR-WP-004` are no longer the bare ground this document once called them, since a status
reader now exists; and the same run showed that reaching a details view is fully covered,
so a fixture whose only novelty is *navigating* somewhere will produce zero gaps.

**This document goes stale by being used.** A scenario that has been run once is a scenario
that has been consumed — re-run the two commands above, check `tests/ui/`, and add a row to
the table when you spend one. Do not trust a named fixture here without that check; the
last two runs both found the named default already implemented.
