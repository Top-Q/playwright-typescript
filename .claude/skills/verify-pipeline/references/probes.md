...
Then read **every** assertion in the final file, not only the added ones — count and delta
both miss the assertion that was dead on arrival. For each one, open the page object and
name the application state that would make it red. The recurring shape is an assertion
restating the wait its own navigation method already performed:

```typescript
detailsPage = await newWorkPackagePage.clickSaveButton();     // returns through waitForLoad()
expect(detailsPage.page.url()).toMatch(/\/work_packages\/details\/\d+/);
```

where `WorkPackageDetailsPage.waitForLoad()` is `await this.page.waitForURL(/\/work_packages\/details\/\d+/)`.
Observed in run `tc-wp-003-01-2026-08-12-14-55-06`, written at stage 1 and missed by the
reviewer. It is not a weakened test — nothing was removed — but it is coverage theatre, and
a confident comment above it claiming the line proves persistence makes it worse, not
better. `should-fix` against `.claude/agents/test-creator.md` and `test-reviewer.md`.
...
## P12 — Cleanup durability: what does a *failed* run leave behind?

Every generated test creates its own data (CLAUDE.md rule 20), and the pipeline's house
style puts the deletion in a trailing `test.step`. That step runs only when every step
before it passed — so the runs that leave data behind are exactly the runs that failed, and
they are also the runs nobody re-reads.

Read the tail of the test file, then check the app:

```bash
node -e "const t=require('fs').readFileSync(process.argv[1],'utf8');
const m=[...t.matchAll(/test\.step\('([^']+)'/g)].map(x=>x[1]);
console.log(m[m.length-1]);" <testFile>
```

- Is the last step a cleanup step? If so, everything it deletes is orphaned on any earlier
  failure — including a failure in the middle of a *healing* run, which is where this
  pipeline spends its red runs.
- Did this run actually orphan something? `build-report.md`'s housekeeping section is where
  po-builder confesses it; the app's own list is the check that does not rely on the
  confession. Run `tc-wp-003-01-2026-08-12-14-55-06` left work package **#169** behind from
  an investigation run, and said so.
- Count the orphans in the project before and after the run. A run that leaves data behind
  changes the fixture for the *next* run, which is how a suite starts depending on
  execution order (rule 19) without any test mentioning another.

The finding is not "the test should not clean up". It is that cleanup keyed to the happy
path is not cleanup. Fixtures with teardown, or an `afterEach`, survive a mid-test failure;
a trailing step does not. File it against `.claude/agents/test-creator.md`, which is where
the pattern is prescribed.

This probe exists for the same reason P11 does: a `test-reviewer` raised it unaided in run
`tc-wp-003-01-2026-08-12-14-55-06` and no probe here would have caught it.
