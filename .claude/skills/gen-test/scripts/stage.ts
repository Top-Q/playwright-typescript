...
/**
 * Time from the run's creation to a stage, as `+4m20s`.
 *
 * Where the run's cost actually went is the question `--list` gets asked, and
 * subtracting two wall-clock stamps by eye is the step everybody skips.
 */
function sinceStart(origin: number, at: string): string {
  const delta = Date.parse(at) - origin;
...
try {
  ...
  if (values.list) {
    ...
    if (values.json) {
      ...
      console.log(`stage: ${record.runId} — ${record.status}  (created ${record.createdAt})`);
      const origin = Date.parse(record.createdAt);
      let previousDay = record.createdAt.slice(0, 10);
      for (const entry of stages) {
        const exit = entry.exitCode === undefined ? '' : ` exit ${entry.exitCode}`;
        // The date is printed only when it changes from the line above. A run
        // paused across midnight otherwise renders 09:09 under 15:11 and reads
        // as though the stages ran out of order.
        const day = entry.at.slice(0, 10);
        const stamp =
          day === previousDay
            ? `${' '.repeat(11)}${entry.at.slice(11, 19)}`
            : `${day} ${entry.at.slice(11, 19)}`;
        previousDay = day;
        console.log(
          `  ${stamp}  ${sinceStart(origin, entry.at).padStart(7)}  ${entry.status.padEnd(4)}  ` +
