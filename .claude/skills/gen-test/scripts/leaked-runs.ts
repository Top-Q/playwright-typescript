 ...
 */
function listProcesses(): { ok: boolean; processes: RawProcess[]; reason?: string } {
  if (process.platform === 'win32') {
    // Attempted twice. The CIM query fails intermittently — a preflight degraded
    // to "could not enumerate processes" while the identical command, run by hand
    // minutes later, returned 126KB of JSON at exit 0. A failure is
    // indistinguishable from "no leaks found" to every caller, so it is worth a
    // second attempt and, when both fail, a reason the caller can print.
    let reason = '';
    for (let attempt = 0; attempt < 2; attempt += 1) {
      ...
      if (query.code !== 0) {
        reason = `query exited ${query.code}: ${query.out.split(/\r?\n/)[0] || 'no output'}`;
        continue;
      }
      // Exit 0 with nothing on stdout is a failed enumeration, not an empty
      // process table: this machine always has processes. Reporting it as a
      // success is how "no leaks" gets claimed without looking.
      if (!query.out) {
        reason = 'query exited 0 but produced no output';
        continue;
      }
      try {
        ...
        reason = `unparseable JSON (${query.out.length} chars): ${
          error instanceof Error ? error.message : 'unknown error'
        }`;
      }
    }
    return { ok: false, processes: [], reason };
  }
  ...
  if (listing.code !== 0) {
    return {
      ok: false,
      processes: [],
      reason: `ps exited ${listing.code}: ${listing.out.split(/\r?\n/)[0] || 'no output'}`,
    };
  }
...
export function findLeakedTestRuns(repoRoot: string, selfPid = process.pid): {
  /**
   * False when the process table could not be read at all. An empty `runs` under
   * `enumerated: false` means "did not look", never "looked and found nothing" —
   * callers must not report a clean state without checking this flag.
   */
  enumerated: boolean;
  /** Why enumeration failed, for the report. Absent when `enumerated` is true. */
  reason?: string;
  runs: LeakedRun[];
} {
  const { ok, processes, reason } = listProcesses();
  if (!ok) return { enumerated: false, reason, runs: [] };
