...
This skill owns the **pipeline**: [references/](references/) for the contract, the gates and the browser recipes, and [scripts/](scripts/) for the programs behind the `npm run pipeline:*` and `npm run gate:*` aliases. Call those through the aliases, never by path — `package.json` is the only file that should know where they live.

It does not own the craft or the application. The numbered rules every stage is held to are in [CLAUDE.md](../../../CLAUDE.md), and what the app under test looks like is in [docs/app-under-test/](../../../docs/app-under-test/). Both are shared with engineers writing page objects by hand, so neither is restated here.

Read [references/gates.md](references/gates.md) for the gate commands and [references/contract.md](references/contract.md) for the artifact schemas. The agents read the contract themselves — you do not need to explain it to them, and the same goes for [docs/app-under-test/environment.md](../../../docs/app-under-test/environment.md) (addresses, credentials, the source checkout), [references/browser.md](references/browser.md) (how to drive the app) and [docs/app-under-test/openproject-dom.md](../../../docs/app-under-test/openproject-dom.md) (what it renders), which the browser-using agents load from their own instructions.
...
The cap exists to protect CLAUDE.md rule 21. Unbounded healing corners an agent into choosing between a red gate and an honest test, and that is a choice no one should have to make: three attempts, then the failure is the deliverable.
...
- **Do not let a stage's self-report substitute for a gate.** An agent saying "all gaps implemented" is a claim; `gate:gaps` is the evidence. _(This was CLAUDE.md rule 24, relocated here 2026-09-06: it governs orchestration, not the repository, so it belongs where the orchestrating happens. The number stays occupied in CLAUDE.md.)_
