...
concern — that lives in [`openproject-dom.md`](../../../../docs/app-under-test/openproject-dom.md), which those agents
read unconditionally. Addresses and credentials live in
[`environment.md`](../../../../docs/app-under-test/environment.md). Do not add OpenProject DOM facts here.
...
> **Version note.** `--debug=cli` requires Playwright **≥ 1.59.0** (bisected 2026-07-23: 1.57.0/1.58.0 boolean-only; 1.59.0+ accept `--debug [mode]` with choices `"inspector"` / `"cli"`). The project runs **1.62.1** (`playwright-cli` 0.1.18). On 1.56.1 it failed with `error: unknown option '--debug=cli'` — if you ever see that, the lockfile has drifted back.

---

## `pause-at` works as of 0.1.18 — with one rule

**Re-verified 2026-08-12** on Playwright 1.62.1 / `playwright-cli` 0.1.18. It pauses, and
you can jump straight to a deep action instead of stepping the whole way.

The rule: **target the line that makes the Playwright call, which here is inside
`src/po/**`, not the spec.** Playwright attributes an action to the innermost user frame,
and our specs only call page-object methods. `pause-at membersPage.ts:364` pauses;
`pause-at <spec>.ts:51` does not. That is what the 2026-07-23 entry here got wrong — the
targets tried were spec lines, and the conclusion "broken" outlived the truth by a
version.

It still **fails open and silently** — a location that matches nothing gives no error, no
output and no pause; the test runs to completion. On a test with side effects, "pause
before the destructive step" then runs the whole thing. This is how a stray member got
invited to the Demo project. A real pause always prints a `### Paused` block; no block
means no pause.

Full rules, worked example and hazards: the [`pause-test`](../../pause-test/SKILL.md) skill.
`step-over` remains correct and is what you want once `pause-at` has landed you nearby.
...
[`openproject-dom.md`](../../../../docs/app-under-test/openproject-dom.md) § Accessible names. They are why step 2 is a
...
faster than reproducing.

When the trace leaves the cause ambiguous, use Recipe A against the failing test and
`pause-at` the failing action — a stale snapshot cannot answer "what does this element do
*now*", and a paused browser can:
...
[`openproject-dom.md`](../../../../docs/app-under-test/openproject-dom.md) — most failures here have happened before.
