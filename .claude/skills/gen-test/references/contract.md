...
```jsonc
{
  ...
  "sourceFile": "specs/product/vault/Requirements/FR-MEM-001.md",
...
`suggestedTestFile` is a suggestion. The test-creator **always** records the path it actually wrote as `testFile`, whether or not it matches the suggestion, because every later stage runs the test from that value:

```bash
npm.cmd run pipeline:set -- --test-file tests/ui/members/invite-a-new-user.spec.ts
```

Unconditionally, not only on deviation. `pipeline:audit` warns whenever `testFile` is unset — a later stage running off a suggestion cannot tell "the creator agreed with it" from "the creator never said" — so recording it only when the path differs makes every otherwise-perfect run produce a warning, and a warning that fires on correct behaviour trains everyone to ignore the ones that matter.
...
[
  {
    ...
    "partiallyExists": "UNVERIFIED: MembersPage.getInviteStatus() reads a sibling region; the banner may follow the same pattern"
  }
]
```
...
`partiallyExists` is optional, and it is the one field that can actively mislead the next stage. It is for "something adjacent exists and here is how it relates" — a related method, a component that already scopes the right region. **The creator has never seen the page**, so anything it says about the DOM is extrapolation from another attribute's page object, and must be prefixed `UNVERIFIED:`. po-builder treats an unprefixed claim as fact and an `UNVERIFIED:` one as a hypothesis to test first.

This is not hypothetical. In run `tc-wp-003-01-2026-08-12-14-55-06` the field asserted that Status and Priority both render as `.inline-edit--container.<attribute>`. True of Priority; false of Status, which is `op-wp-status-button` with a fixed aria-label and an `aria-hidden` value span — a locator built from the claim matches **zero** elements. po-builder caught it against the Rails source, which is the design working, but it spent a stage on it.
