...
Writing page objects here means writing locators by hand, without the pipeline's evidence gates. Read
[`docs/app-under-test/openproject-dom.md`](../../../docs/app-under-test/openproject-dom.md) first —
what the app renders, and where the naive locator is wrong — and
[`docs/app-under-test/environment.md`](../../../docs/app-under-test/environment.md) for addresses,
credentials and the Rails source.
...
If the page state you care about occurs mid-test, attach to *that* test and `pause-at` it. That works as of `playwright-cli` 0.1.18, but the target must be a **page-object** line, not a spec line, and a wrong location fails open — no error, no pause, the test runs to completion. See the `pause-test` skill (`.claude/skills/pause-test/SKILL.md`).
