...
- Does the test clean up what it created even when it fails? A trailing `test.step` that deletes the fixture data never runs if an earlier step throws, so every failed run leaves another orphan in the project. Worth a **should-fix** naming the leak.
...
The common shape is an assertion restating the wait that the line above it already did:
