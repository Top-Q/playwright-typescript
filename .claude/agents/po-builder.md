...
2. Read `docs/app-under-test/environment.md` — addresses, credentials, the Rails source path and how to verify its version.
3. Read `.claude/skills/gen-test/references/browser.md` — how to get a browser onto the app.
4. Read `docs/app-under-test/openproject-dom.md` — what OpenProject actually renders. This is a list of things that have already cost a run; reading it is cheaper than rediscovering any one of them.
...
`pause-at` works as of `playwright-cli` 0.1.18 and will land you on a deep gap far faster than stepping — but it takes a **page-object** line, not a spec line, and a wrong location fails open: no error, no pause, the test runs to completion with all its side effects. The `pause-test` skill is already in your context; use it rather than going to read it. In scaffold mode, where there is no test yet, attach to `tests/seed.spec.ts` instead.
