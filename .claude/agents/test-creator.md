...
- `tests/ui/<module>/<kebab-name>.spec.ts`. Record the path you actually wrote with `npm.cmd run pipeline:set -- --test-file <path>` — **always**, even when it is identical to `suggestedTestFile`, because every later stage runs that value and an unset `testFile` is audited as "running off a suggestion". Do not hand-edit `run.json`.
...
- **Every assertion must be able to fail.** Before you write one, name the state of the application that would make it red. If you cannot, delete it.

The last one is the easiest to get wrong, because the assertion that cannot fail looks exactly like a careful one:

```typescript
// WRONG - clickSaveButton() returns through waitForLoad(), which already awaited
// this same URL pattern. No application bug makes this line red.
...
Navigation methods return through `waitForLoad()`, so **whatever `waitForLoad()` waits for is already guaranteed by the time you get the object back** — a URL, a heading, a spinner that cleared. Read it before you assert (CLAUDE.md rule 16) and assert something it does not cover: the data the step was actually about. A step whose whole outcome is "we arrived" needs no assertion at all — arriving is the assertion, and it is enforced by the timeout. Leave it bare and say why in a comment.
...
```typescript
// The body is SYNCHRONOUS - no `async`. An async body containing only a throw
// trips require-await, which is an error here, so it cannot pass gate:lint.
...
That is the whole mechanism. No method call, no method name, no page-object edit. Keep the marker on one line and inside 100 columns — `check-gaps` matches it per line, so a Prettier-wrapped throw is invisible to the gate and the run scores zero gaps.
