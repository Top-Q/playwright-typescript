...
skills: pause-test, playwright-trace
model: inherit
---
...
2. Read `docs/app-under-test/openproject-dom.md`. Read it **now**, before you look at the failure — not later as a tie-breaker. Most failures in this app are a repeat of something on that list, and knowing the list changes what you notice in the trace. Reading it after you have formed a theory is worth far less.
3. Read `docs/app-under-test/environment.md` — addresses, credentials, and the Rails source path with its version check, for when a locator needs checking against what the app is built from.
...
If the trace is not enough, reproduce it live with **Recipe A** in `.claude/skills/gen-test/references/browser.md` — attach to the failing test itself, not to a seed — and `pause-at` the failing action.

**The `pause-test` skill is already in your context** — you do not need to go and read it. `pause-at` works as of `playwright-cli` 0.1.18, so you can land on a deep failure directly instead of stepping the whole way — but it takes a **page-object** line, not a spec line, and a wrong location fails open: no error, no pause, the test runs to completion with all its side effects.
...
playwright-cli --s=tw-XXXXXX pause-at "<pageObject>.ts:<line>"             # the line with the real call
playwright-cli --s=tw-XXXXXX step-over                                     # one action at a time from there
...
No `### Paused` block back from `pause-at` means it did not pause. Do not carry on as if it had.
