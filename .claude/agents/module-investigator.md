...
1. Read `docs/app-under-test/environment.md` — addresses, credentials,
   the Rails source path, and how to verify its version.
2. Read `docs/app-under-test/openproject-dom.md` — what this app
...
`pause-at` works as of `playwright-cli` 0.1.18, but it takes a **page-object** line, not a
spec line, and a wrong location fails open — no error, no pause, the test runs on. Rules
and hazards are in the `pause-test` skill, already in your context. For seed, four
`step-over`s are simpler
than a pause target, and seed has no side effects, so a mis-step costs nothing; on any
other test it would not be free.
